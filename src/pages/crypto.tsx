...
import { useAlpaca } from "../providers/AlpachaProvider"; 
import '../styles/stocks.css';
import { ChartData } from '../types/types';
import { OpenAiCall } from '../api/Open_AIService';
...
//   );
  ...
  const formatDataForChart = (lastJsonMessage: ChartData[]) => {
    ...
    const newData = lastJsonMessage.map((item) => ({
      name: item.t,   // X-axis label
      uv: item.c,     // closing price 
      pv: item.v,     // volume 
    }));
    ...
    return newData
     ...
     const makeACall = async () => {
      try {
        const response = await OpenAiCall(tickers);
        console.log("response", response.data.args.companyHistory);
        const companyHistory = response.data.args.companyHistory;
       return companyHistory
      } catch (error) {
        console.error("Error during OpenAiCall:", error);
      }
    };
...
useEffect(() => {
  ...
  const handleButtonClick = () => {
    // correctly modify the input
    const tickersArray = input.split(/\s*,\s*/); 
